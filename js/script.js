...
function iniciar() {
    ...
    let seccionesTotal = document.querySelectorAll("main > div").length - 1;
...
function calculoPosicionNav(array, position) {
    let calculoDiv = Math.abs((scrollDoc) / (array[position] - docHeight));
    return Math.round(calculoDiv*100); 
}
